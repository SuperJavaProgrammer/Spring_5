...
//import webmvc.services.SingerService;
//import webmvc.util.Message;
//import webmvc.util.SingerGrid;
//import webmvc.util.UrlUtil;
...
@RequestMapping("/singers") //задает корневой URL, который будет обрабатываться контроллером
...
public class SingerController {
    ...
    @RequestMapping(method = RequestMethod.GET) //метод сопоставляется с методом доступа GET
    public String list(Model uiModel) {
        logger.info("Listing singers");
        List<Singer> singers = singerService.findAll();
        uiModel.addAttribute("singers", singers); //список певцов сохраняется в Model
        logger.info("No. of singers: " + singers.size());
        return "singers/list"; //возвращается имяредставления. Файл для представления - /WEB-INF/views/singers/list.jspx
    }
    ...
    public String update(@Valid Singer singer, BindingResult bindingResult, Model uiModel, //@Valid - проверка по валидации по указанным параметрам в Singer
