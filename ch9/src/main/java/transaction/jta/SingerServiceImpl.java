...
public class SingerServiceImpl implements SingerService {
	...
	public List<Singer> findAll()
	{
		...
		if (singersFromA.size() != singersFromB.size()){
			throw new RuntimeException("XA resources obj not contain the same expected data.");
		}
	...
	public Singer save(Singer singer) {
		...
		if (singer.getId() == null) {
			emA.persist(singer);
//			if(true) { //чтобы сэмитировать неудачное выполнение, поэтому будет откат, можно проверить и убедиться в этом
